...
async function startServer() {
  ...
  wss.on('connection', (ws: WebSocket, req) => {
    ...
    ws.send(JSON.stringify({
      type: 'system:init',
      server: 'gemini-vscode-agent-bridge',
      version: '1.2.0',
      workspace: path.basename(workspaceRoot),
      message: 'Connected to Gemini Web & VS Code Bridge Server.',
  ...
  app.get('/api/extension/download-chrome-zip', async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="gemini-chrome-extension.zip"');
      res.send(zipBuffer);
    } catch (err: any) {
  ...
  app.get('/api/extension/download-suite-zip', async (req, res) => {
    try {
      ...
      res.setHeader('Content-Type', 'application/zip');
      res.setHeader('Content-Disposition', 'attachment; filename="gemini-vscode-agent-suite.zip"');
      res.send(zipBuffer);
    } catch (err: any) {
...
\`\`\`json
{
  ...
  "args": {
    "path": "src/example.ts",
    "content": "// Generated by Gemini VS Code Agent\\nexport function helloWorld(): string {\\n  return 'Hello from Gemini VS Code Agent!';\\n}\\n\\nconsole.log(helloWorld());"
  }
}
\`\`\``;
    } else {
      simulatedResponse = `I am your Gemini VS Code Autonomous Agent. I am ready to inspect files, write code, or execute terminal commands in your workspace.
...
- "Run npm test"`;
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Gemini VS Code Agent Server running on http://0.0.0.0:${PORT}`);
